...
class KlaroWebsite(models.Model):
    ...
    klaro_embedded = fields.Boolean(
        string="Embedded",
        default=False
    )
    klaro_notice_as_modal = fields.Boolean(
        string="Notice as modal",
        default=False
    )
    klaro_no_autoload = fields.Boolean(
        string="No autoload",
        readonly=False
    )

    klaro_group_by_purpose = fields.Boolean(
        string="Group by purpose",
        default=True
    )
    ...
    ], string='Horizontal Position', default='right')

    klaro_wide = fields.Boolean(
        string="Wide",
        default=False
    )

    klaro_theme_light = fields.Boolean(
        string="Light Theme",
        default=True
    )
    ...
    def get_klaro_config(self):
        ...
        for service in self.klaro_service_ids:
            ...
            if service.on_init:
                service_config.update({
                    'onInit': service.on_init
                })                
            if service.on_accept:
                service_config.update({
                    'onAccept': service.on_accept
                }) 
            if service.on_decline:
                service_config.update({
                    'onDecline': service.on_decline
                }) 
